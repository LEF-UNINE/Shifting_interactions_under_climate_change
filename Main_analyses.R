...
# Import growth data
Diapause <- read.csv("Diapause.csv", h = T, sep = ";")
Diapause$Plant <- factor(Diapause$Plant, levels =  c("P. lanceolata", "P. atrata"))

# Data visualization:
# - - - -  - - - - - -

# Diapause (= 1-pupation) probability 
sum_up_data3 <- summarySE((Diapause), measurevar="Diapause", groupvars=c("Plant"),na.rm = TRUE)
pupation <-ggplot(sum_up_data3, aes(x=Plant, y=Diapause, fill=Plant, colour=Plant)) + 
  geom_errorbar(aes(ymin=Diapause-se, ymax=Diapause+se, color=Plant), width=0.2, size=1.4, position=position_dodge(0.5)) +  
...
# Statistical analyses of the pupation rates
# - - - - - - - - - - - - - - - - - - - - - -

# Diapause/Pupation model

# Built a Binomial GLM model for diapause probability
d1 <- glm(Diapause ~ Plant, data= Diapause, family = binomial)
